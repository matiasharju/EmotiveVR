	...
	SubShader
	{
		Pass
		{
			...
			float mandelboxDE(float3 testPoint, int iterations) {
				...
				return box(p, float3(0.1, 8.1, 8.1) *_knob7) / abs(derivative);
			}
			...
			float distanceEstimator(float3 rayPosition, bool calculatingShadows) {
                ...
                if (element2 < element1) {
					...
					surfaceColor.b -= 0.95 * (1 - 0.96 * step(0.25, trap.z));
					//surfaceColor.r = surfaceColor.g;
                }
			...
			fixed4 frag (v2f input) : SV_Target {
				...
				if (!didHitSurface && stepNumber < maxSteps) {
					...
					return float4(
						pow(raymarchLight,4.1) * 0.2,
						pow(raymarchLight, 0.4) * 0.7,
						pow(raymarchLight, 0.9) * 0.7,
						1);
				}
