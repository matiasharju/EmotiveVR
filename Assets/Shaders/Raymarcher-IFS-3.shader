		...
		Pass
		{
			...
			float3 foldIterator(float3 p) {
				for (int i = 0; i < _iterations; i++) {
					...
					p -= float3(0, 0.02, 0);
					if (i%3 == 2) {
						foldNormal = float3(cos(_foldRotateXY * 0.5), 
									 		sin(_foldRotateXY * 0.1),
									 		0);
			 		} else {
			 			foldNormal = float3(0,
									 		-sin(_foldRotateYZ * 0.1),
									 		cos(_foldRotateYZ * 0.1));
					}
			...
			fixed4 frag (v2f input) : SV_Target
			{
                ...
                if (elementID == 1) {
                    ...
                    return float4(brightness * 0.1, brightness * 0.1, brightness * 0.1,3);
                }
			}
			ENDCG
		}
