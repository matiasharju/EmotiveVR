...
Shader "Unlit/RM-shaded-terrain"
{
	Properties
	{
	...
	SubShader
	{
		Pass
		{
			...
			float distanceEstimator(float3 rayPosition, bool calculatingShadows) {
				...
				float an2 = sin(_Time * 10);
				float an3 = _Time * 20;
                float element1 = sphere(rayPosition - float3(-16.0 - 0*an2, 1*an + 1, 18 + 0*an2), 4);
                float element2 = 10000;
				//float elementSlab = plane(rayPosition - float3(0, -4, 0), float4(0.0, 1.0, 0.0, 0.0)) - displace3dSinWaves(float3( rayPosition.x + an3, rayPosition.y, rayPosition.z), 2);
				float elementSlab = plane(rayPosition - float3(0, -4, 0), float4(0.0, 1.0, 0.0, 0.0))	- valueNoise(rayPosition)/3 ;
				//float elementSlab = terrain(rayPosition.xz);

				float3 rightHandOffset = mul(UNITY_MATRIX_V, _rightHandPosition).xyz;
				float rightHandObject = sphere(rayPosition - _WorldSpaceCameraPos - _rightHandPosition + float3(0.0,1.7,0.0), 0.05);
			...
			fixed softCastShadows(float3 p) {
				...
				float lightDistanceFromSurface = 50; // just put some large value so we enter the iteration
		            ...
		            float lightTravelDistance = 1;
		            for (float t = lightTravelDistance; t < lightDistanceFromSurface;) {
		                float h = distanceEstimator(p + lightDirection * lightTravelDistance, true);
						if (h < 0.001) {
							return 1.0;
						}
						shadow = min(shadow, shadowHardness * h / t );
		                t += h;
			...
			fixed4 frag (v2f input) : SV_Target {
				...
				float stepNumber = 0;
				float travelDistance = 0.5;  // Normally 0.0 but bigger number creates protective bubble around the camera
				float3 rayPosition;
			    ...
			    float ambientOcclusion = pow(log(float(stepNumber)) / log(float(maxSteps)), 3.0);
				float ambientLight = 0.03;

				//raymarchLight = log(stepNumber) / log(maxSteps);
				raymarchLight = pow(1 - raymarchLight, 1.1);
				float shadows = ambientOcclusion + softCastShadows(rayPosition);
				shadows = 1.0 * min(1.0, shadows);
				//float shadows = 1 - (1 - ambientOcclusion) * (1 - hardCastShadows(rayPosition, surfaceNormal));
