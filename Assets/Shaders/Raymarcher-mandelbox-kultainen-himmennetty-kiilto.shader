	...
	SubShader
	{
		Pass
		{
			...
			float mandelboxDE(float3 testPoint) {
				...
				float t = pow(sin(_Time.y * 0.03), 3);
			...
			fixed4 frag (v2f input) : SV_Target {
				...
				while ((stepNumber == 0) || (travelDistance < maxDistance && distanceToSurface > touchDistance && stepNumber < maxSteps)) {
					stepNumber += 1;
					rayPosition = eyePosition + travelDistance * rayDirection;
				...
				bool didHitSurface = distanceToSurface <= touchDistance * 20;
...
//				brightness = pow(brightness * b, 2);
				...
				//brightness = 0;
				float3 color1 = float3(brightness, brightness, brightness);
				color1 = float3(pow(brightness, 0.5), pow(brightness, 0.7), pow(brightness, 1.7));
				...
				// reflection
				eyePosition = rayPosition - rayDirection * touchDistance * 30;
				float3 surfaceNormal = accurateSurfaceNormalNearPoint(eyePosition, touchDistance);
				...
				didHitSurface = distanceToSurface <= touchDistance * 20;
				float3 color2;

				brightness = stepNumber/maxSteps;
				if (!didHitSurface) {
					brightness = 1 - brightness;
					brightness = pow(brightness, 1.4);
					color2 = float3(pow(brightness, 2), pow(brightness, 0.9), pow(brightness, 0.7));
				} else {
					brightness = log(stepNumber) / log(maxSteps);
					brightness = pow(1-brightness, 1);
					brightness = pow(brightness, exp(-(brightness*2-1)));
					color2 = float3(pow(brightness, 0.5), pow(brightness, 0.7), pow(brightness, 1.7));
				}

				color2 = pow(color2, 3) * 0.4;
				...
				return float4(1 - (1-color1.x) * (1-color2.x),
							  1 - (1-color1.y) * (1-color2.y),
							  1 - (1-color1.z) * (1-color2.z), 1);
