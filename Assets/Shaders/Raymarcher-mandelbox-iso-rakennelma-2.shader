	...
	SubShader
	{
		Pass
		{
			...
			float mandelboxDE(float3 testPoint) {
				...
				return box(p, float3(0.5, 0.5, 1.5)) / abs(derivative);
			}
			...
			float distanceEstimator(float3 position) {
				...
				float overShoot = 1;
				float3 p = position - float3(-65, 0, -75);
				float zoom = 20;
				p /= zoom;
				p -= float3(4.3, 0.5, 5.5);
				float element1 = mandelboxDE(p) * zoom * overShoot;

				float element2 = box(position - float3(0.1, -0.25, 1.8), float3(4.0, 0.2, 4.0));
