...
Shader "Unlit/Raymarcher-Mandelbox-3"
{
	Properties
	{
	...
	SubShader
	{
		Pass
		{
			...
			float distanceEstimator(float3 position) {
				...
				float3 p = position - float3(0, 0, 0);
				float zoom = 15;
				p /= zoom;
				p -= float3(4.3, 0.5, 5.5);
                float element1 = mandelboxDE(p, 2) * zoom * overShoot;
                float element2 = mandelboxDE(p, 5) * zoom * overShoot;
				float elementSlab = box(position - float3(0.1, -0.25, 1.8), float3(4.0, 0.2, 4.0));
                float frontElement = element1;

                elementID = 1;
                surfaceColor = float3(0.1, 0.45, 0.05);
                ...
                if (element2 < element1) {
                    ...
                    //float colorModifier = 1 - 2 * smoothstep(-0.02, 0.02, sin(20.0 * (p.y + p.x)));
                    surfaceColor = float3(1.0, 1.0 * colorModifier, 1.0 * colorModifier);

                }
                if (elementSlab < frontElement) {
                    elementID = 3;
                    surfaceColor = float3(0.0, 0.0, 1.0);
                    return elementSlab;
                }
