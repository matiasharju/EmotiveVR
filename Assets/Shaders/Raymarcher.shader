...
Shader "Unlit/Raymarcher"
{
	...
	SubShader
	{
		Pass
		{
			...
			float torusDE(float3 p, float t) {
				float2 q = float2(length(p.xz)-t, p.y);
				return length(q)-t;
			}
			...
			float mandelboxDE(float3 testPoint) {
				...
				//return sphereDE(p, 0.8) / abs(derivative);
				return torusDE(p, float(2)) / abs(derivative);
			}
			...
			float distanceEstimator(float3 position) {
				...
				float overShoot = 2;
				float3 p = position - float3(-60, -45, -85);
				float zoom = 20;
				p /= zoom;
			...
			fixed4 frag (v2f input) : SV_Target {
				...
				bool didHitSurface = distanceToSurface <= touchDistance * 7; //10
				...
				return float4(brightness, brightness * 0.0, brightness * 0.0, 1);
			}
			ENDCG
		}
	}
