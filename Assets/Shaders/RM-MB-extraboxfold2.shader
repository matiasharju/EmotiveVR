	...
	SubShader
	{
		Pass
		{
			...
			float mandelboxDE(float3 testPoint, int iterations) {
				...
				return box(p, (1.1, 1.1, 4.1)) / abs(derivative);
			}
			...
			float distanceEstimator(float3 rayPosition, bool calculatingShadows) {
                ...
                float element2 = mandelboxDE(mul(p + float3(0,-5.5,0), rotateXY(_Time*2)) , _iterations) * zoom * overShoot ;//+ min(p.y < h ? 1 : 0, 0.03 * smoothstep(-0.7, -0.65, sin(400.0 * (p.y ))-1.2));
