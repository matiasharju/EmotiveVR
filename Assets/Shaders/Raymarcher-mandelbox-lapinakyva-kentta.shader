		...
		Pass
		{
			...
			float mandelboxDE(float3 testPoint) {
				...
				for (int i = 0; i < 1; i++) {
					...
					if (length(p) < 0.5) {
						derivative *= 20; // glitch, should be 4
						p *= 4;
					} else if (length(p) < 1) {
						derivative /= dot(p, p);
						p /= dot(p, p);
