	...
	SubShader
	{
		Pass
		{
			...
			float mandelboxDE(float3 testPoint, int iterations) {
				...
				float foldAngle = 3.1415 * 0.01 * _foldRotateXY;
				int iterationModulo = 7;
				int element1; // for mixing shapes
				int element2; // for mixing shapes

				for (int i = 0; i < iterations; i++) {
					float3 foldNormal = float3(sin(foldAngle), cos(foldAngle), 0);
			        p -= 2.0 * min(0.0, dot(p, foldNormal)) * foldNormal; // a fold in a general plane with normal n
					//foldAngle = 3.1415 * sin(_Time.y / 300);
			        float s;
			        float t = 0;
			        if (i % iterationModulo == 0) {
						...
						foldAngle = 3.1415 * 0.01 * _foldRotateXZ;
						foldNormal = float3(sin(foldAngle), cos(foldAngle), 0);
						p -= 2.0 * min(0.0, dot(p, foldNormal)) * foldNormal; // a fold in a general plane with normal
						//p = opU(sphere(p, 1), box(p,1));
			        } else if (i % iterationModulo == 1) {
			            s = s2;
			            t = t2;
			            p = float3(p.y, p.z, p.x);
						foldAngle = 3.1415 * 0.01 * _foldRotateYZ;
						foldNormal = float3(sin(foldAngle), cos(foldAngle), 0);
			            p -= 0.2 * min(0.0, dot(p, foldNormal)) * foldNormal; // a fold in a general plane with normal
						//p = opU(sphere(p, 1), box(p,1));
				...
				return box(p, float3(1, 0.1, 1)*_knob7) / scale;
			...
			float distanceEstimator(float3 rayPosition, bool calculatingShadows) {
				...
				float element1 = mandelboxDE(p, _iterations - 2) * zoom * overShoot ;
				float element2 = opI(mandelboxDE(p, _iterations) * zoom * overShoot, element1);
				element1 = 10000;
                ...
                if (element2 < element1) {
                    ...
                    colorModifier = 1 - 0.96 * step(0.05, trap.x);
			...
			fixed4 frag (v2f input) : SV_Target {
				...
				float surfaceLight = ambientLight + max(0, raymarchLight * (0.4 + 0.6 * surfaceLighting(rayPosition, surfaceNormal)) - shadows);
